...
def build_dataframe(duration_data: dict, transition_data: dict, status_rules: Dict[str, List[str]]) -> pd.DataFrame:
    ...
    for row in duration_data.get("table", {}).get("body", {}).get("rows", []):
        ...
        # Capture IssueType and Sprint from fieldColumns
        issue_type = next((c.get('value') for c in row.get('fieldColumns', []) if c.get('id') == 'issuetype'), None)
        sprints = next((c.get('value') for c in row.get('fieldColumns', []) if c.get('id') == 'customfield_10021'), None)
        record['IssueType'] = issue_type
        if isinstance(sprints, list):
            record['Sprint'] = ', '.join(sprints)
        else:
            record['Sprint'] = sprints
        total_days = 0
