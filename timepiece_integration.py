...
def extract_field(row, field_id):
    val = next((c.get('value') for c in row.get('fieldColumns', []) if c.get('id') == field_id), None)
    if isinstance(val, list):
        return ', '.join(v.get('value') for v in val if isinstance(v, dict) and v.get('value'))
    return val
...
def build_dataframe(duration_data: dict, transition_data: dict, status_rules: Dict[str, List[str]]) -> pd.DataFrame:
    ...
    for row in duration_data.get("table", {}).get("body", {}).get("rows", []):
        ...
        record['IssueType'] = extract_field(row, 'issuetype')
        record['Sprint'] = extract_field(row, 'customfield_10021')
        total_days = 0
