...
# ---------- SPRINT ANALYSIS ----------
# Use the last tab we just added to render the analysis
try:
    sprint_tab = tabs[-1]
except Exception:
    sprint_tab = None
...
if sprint_tab is not None:
    with sprint_tab:
        ...
        # We expect a filtered dataframe named view_df (used across other tabs)
        ...
        if df_src is None or df_src.empty or "Sprint" not in df_src.columns or "IssueType" not in df_src.columns:
            ...
            if expanded.empty:
                ...
                # Infer sprint recency using the latest End date in each sprint
                ...
                # Sort sprints by recency
                merged["Sprint"] = pd.Categorical(merged["Sprint"], categories=sprint_order, ordered=True)
                merged = merged.sort_values(["Sprint", "IssueType"]).reset_index(drop=True)
