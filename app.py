...
with st.sidebar:
    ...
    ct_source = st.radio(
        "Source",
        options=["Dates (Started/Finished)", "Auto (Started/Finished can be dates or durations)", "Duration text column(s)"],
        index=0,
        help="Use normal dates, auto-parse Started/Finished as dates or duration text, or sum duration columns."
    )
...
def coerce_datetime_or_duration(series: pd.Series):
    """Return tuple (dt, td) where dt is datetime64[ns] (or NaT) and td is Timedelta (or NaT)."""
    dt = auto_to_datetime(series)
    td = pd.Series(pd.NaT, index=series.index, dtype="timedelta64[ns]")
    mask = dt.isna()
    if mask.any():
        parsed = series[mask].apply(parse_duration_text)
        td.loc[mask] = parsed
    return dt, td
...
def compute_ct_flexible(df: pd.DataFrame, started_col: str, finished_col: str, ct_mode: str, round_mode: str):
    ...
    # If Ceil to days selected, cast CT to whole numbers
    if round_mode == "Ceil to days":
        out["CT"] = pd.to_numeric(np.ceil(out["CT"]), errors="coerce")

    return out, bucketer

    """Parse '0d 2h 36m 50s', '2h 5m', '30m', '45s', '1d' into Timedelta. Returns NaT on failure.
       If numeric, treated as seconds.
    """
    if pd.isna(val):
        return pd.NaT
