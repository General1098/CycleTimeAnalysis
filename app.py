...
with st.sidebar:
    ...
    ct_source = st.radio(
        ...
        options=[
            ...
            "Auto (Started/Finished can be dates or durations)",
            "Duration text column(s)"
        ],
        index=0,
        help=(
            "Choose how CT is computed:\n"
            "• Dates: normal Started/Finished.\n"
            "• Auto: Started/Finished may be dates or duration text like '0d 2h 20m 10s'.\n"
            "• Duration columns: sum one/more duration-text columns."
        )
    )

    st.markdown("### Rounding (for any duration-based CT)")
    round_mode = st.selectbox(
        "Rounding (applies to duration-based CT)",
        options=["None", "Ceil to hours", "Ceil to days"],
        index=2,  # default: Ceil to days
        help="If CT is derived from durations, round the result up."
    )

    st.markdown("### Date-based CT mode")
    ct_mode = st.radio(
        "When using dates, compute as…",
        options=["Business days (Mon–Fri)", "Calendar days (elapsed)"],
        index=0,
        help=("Business days excludes weekends (Mon–Fri). "
              "Calendar days counts elapsed days, including start & finish.")
    )

    # Duration columns UI (only used in 'Duration text column(s)' mode)
    duration_cols = []
    ...
    if ct_source == "Duration text column(s)":
        ...
        duration_cols = st.multiselect(
            ...
            options=str_cols,
            help="Values like '0d 2h 36m 50s'. You can pick multiple; they will be added together per row."
        )
        ...
        bucket_date_col = st.selectbox(
            ...
            help="Used to place items on a timeline for charts/forecasts when using duration columns."
        )

    # Auto mode bucketing fallback
    ...
    if ct_source.startswith("Auto"):
        ...
        bucket_date_col_auto = st.selectbox(
            ...
            help="If Started/Finished are durations (no real dates), choose a column to place items on the timeline."
        )
...
def coerce_datetime_or_duration(series: pd.Series):
    """Return (dt, td): dt is datetime64[ns], td is timedelta64[ns] (NaT where not applicable)."""
    dt = auto_to_datetime(series)
    needs_td = dt.isna()
    td = pd.Series(pd.NaT, index=series.index, dtype="timedelta64[ns]")
    if needs_td.any():
        parsed = series[needs_td].apply(parse_duration_text)
        td.loc[needs_td] = parsed
    return dt, td
...
def compute_ct_flexible(df: pd.DataFrame, started_col: str, finished_col: str, ct_mode: str, round_mode: str):
    out = df.copy()

    if started_col == "(none)" and finished_col == "(none)":
        out["CT"], out["_date_issue"] = np.nan, False
        out["CT_business"], out["CT_calendar"] = np.nan, np.nan
        return out, pd.Series(pd.NaT, index=out.index)

    start_dt, start_td = coerce_datetime_or_duration(out[started_col]) if started_col != "(none)" else (pd.Series(pd.NaT, out.index), pd.Series(pd.NaT, out.index))
    finish_dt, finish_td = coerce_datetime_or_duration(out[finished_col]) if finished_col != "(none)" else (pd.Series(pd.NaT, out.index), pd.Series(pd.NaT, out.index))

    both_dt = start_dt.notna() & finish_dt.notna()
    both_td = start_td.notna() & finish_td.notna()
    only_finish_td = finish_td.notna() & start_dt.isna() & start_td.isna() & finish_dt.isna()

    out["CT_business"], out["CT_calendar"], out["CT"] = np.nan, np.nan, np.nan
    out["_date_issue"] = False

    if both_dt.any():
        sD = start_dt[both_dt].dt.floor("D").to_numpy(dtype="datetime64[D]")
        fD = finish_dt[both_dt].dt.floor("D").to_numpy(dtype="datetime64[D]")
        with np.errstate(invalid="ignore"):
            ct_bus = np.busday_count(sD, fD).astype("float")
        ct_cal = (finish_dt[both_dt] - start_dt[both_dt]).dt.days + 1
        out.loc[both_dt, "CT_business"] = pd.to_numeric(ct_bus, errors="coerce").mask(ct_bus < 0)
        out.loc[both_dt, "CT_calendar"] = pd.to_numeric(ct_cal, errors="coerce").mask(ct_cal < 0)
        out.loc[both_dt, "CT"] = out.loc[both_dt, "CT_business" if ct_mode.startswith("Business") else "CT_calendar"]

    if both_td.any():
        delta_sec = (finish_td[both_td] - start_td[both_td]).dt.total_seconds()
        if round_mode == "Ceil to hours":
            delta_sec = np.ceil(delta_sec / 3600.0) * 3600.0
            ct_days = (delta_sec / 86400.0).astype(float)
        elif round_mode == "Ceil to days":
            ct_days = np.ceil(delta_sec / 86400.0).astype(float)
        else:
            ct_days = (delta_sec / 86400.0).astype(float)
        ct_days = pd.to_numeric(ct_days, errors="coerce").mask(ct_days < 0)
        out.loc[both_td, "CT"] = ct_days

    if only_finish_td.any():
        delta_sec = finish_td[only_finish_td].dt.total_seconds()
        if round_mode == "Ceil to hours":
            delta_sec = np.ceil(delta_sec / 3600.0) * 3600.0
            ct_days = (delta_sec / 86400.0).astype(float)
        elif round_mode == "Ceil to days":
            ...
            ct_days = (delta_sec / 86400.0).astype(float)
        ct_days = pd.to_numeric(ct_days, errors="coerce").mask(ct_days < 0)
        out.loc[only_finish_td, "CT"] = ct_days

    invalid = out["CT"].isna()
    out.loc[invalid, "_date_issue"] = True

    bucketer = pd.Series(pd.NaT, index=out.index)
    have_finish_dt = finish_dt.notna()
    have_start_dt = start_dt.notna()
    bucketer.loc[have_finish_dt] = finish_dt.loc[have_finish_dt]
    bucketer.loc[~have_finish_dt & have_start_dt] = start_dt.loc[~have_finish_dt & have_start_dt]
    return out, bucketer
...
if ct_source == "Dates (Started/Finished)":
...
elif ct_source == "Auto (Started/Finished can be dates or durations)":
    raw, bucketer = compute_ct_flexible(st.session_state["data"], started_col, finished_col, ct_mode, round_mode)
    raw = apply_team_mapping(raw, built_in_team_col, key_col, rules_text)
    # If bucketer is all NaT and fallback selected, use it
    try:
        if pd.isna(bucketer).all() and bucket_date_col_auto and bucket_date_col_auto != "(none)" and bucket_date_col_auto in raw.columns:
            bucketer = auto_to_datetime(raw[bucket_date_col_auto])
    except Exception:
        pass
    ct_label = f"Auto (dates or durations), rounding: {round_mode}"
else:
    ...
    if round_mode == "Ceil to hours":
        ...
        ct_values = (tot / 86400.0).astype(float)
    elif round_mode == "Ceil to days":
        ct_values = np.ceil(tot / 86400.0).astype(float)
    else:
        ct_values = (tot / 86400.0).astype(float)
    raw["CT"] = pd.to_numeric(ct_values, errors="coerce")
    raw["_date_issue"] = ~any_parsed
    raw = apply_team_mapping(raw, built_in_team_col, key_col, rules_text)
    raw["CT_business"] = np.nan
    raw["CT_calendar"] = np.nan
    ct_label = f"Duration columns ({' + '.join(duration_cols)}), rounding: {round_mode}"
...
st.caption(f"CT source: **{ct_source}** • {ct_label}")
...
with tabs[1]:
    ...
    if not roll_sel.empty and roll_sel["Month"].notna().any():
        ...
        st.info("No dated rows to chart for this scope. If you are in **Auto** or **Duration** mode with duration-only rows, pick a bucketing date column in the sidebar.")
