...
def fib_overlay_chart(dates, values, title="P85 Cycle Time with Fibonacci Bands"):
    ...
    # Shaded zones
    ax.axhspan(fib_vals[2], fib_vals[0], color="#ffebee", alpha=3.0)   # red zone (worst)
    ax.axhspan(fib_vals[3], fib_vals[2], color="#fff8e1", alpha=3.0)   # orange
    ax.axhspan(fib_vals[4], fib_vals[3], color="#f1f8e9", alpha=3.0)   # yellow/green
    ax.axhspan(fib_vals[-1], fib_vals[4], color="#e8f5e9", alpha=3.0)  # green (best)
