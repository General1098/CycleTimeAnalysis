...
def timepiece_section(opts):
    ...
    filter_id=None; project_key=None; jql=None
    if preset=="Saved filter (filterId)":
        filter_id=st.number_input("Filter ID",1,100000000,12345); params.update({"filterType":"jqlfilter","filterId":int(filter_id)})
    elif preset=="Project key":
        project_key=st.text_input("Project Key","YOURPROJ"); params.update({"filterType":"project","projectKey":project_key})
    else:
    ...
    if test_btn:
        try:
            ...
            client.aggregation_json(test_params)
    ...
    def handle_df(df: pd.DataFrame):
        ...
        # Map numeric columns to cycle time
    ...
    def fetch_as_json(p):
        data=client.aggregation_json(p)
    ...
    if fetch_csv:
        try:
            ...
            # Automatic JSON fallback for diagnostics
            try:
