...
from timepiece_client import TimepieceClient, AttemptLog
...
def timepiece_section(opts):
    st.subheader("Timepiece (Diagnostics-enabled)")
    ...
    auth_mode=col0.selectbox("Auth",["Header","Query"],0)
    send_params=col1.selectbox("Send params",["JSON body","Query string","Form-encoded"],0)
    aggregation_type=col2.selectbox("aggregationType",["average","sum","median","standardDeviation"],0)
    safe_mode=col3.checkbox("Safe Mode",True)
    preset=st.radio("Source",["Saved filter (filterId)","Project key","Custom JQL"],0,horizontal=True)
    c1,c2=st.columns(2)
    page_size=c1.number_input("Page size",1,1000,25 if safe_mode else 100)
    ...
    if preset=="Saved filter (filterId)":
        fid=st.number_input("Filter ID",1,100000000,10547); params.update({"filterType":"jqlfilter","filterId":int(fid)})
    ...
    client=TimepieceClient(api_host,token,auth_mode=("query" if auth_mode=="Query" else "header"),send_params=send_params)
    st.divider()
    ...
    if cC.button("Probe Calendar Settings"):
        ok, res = client.probe_calendar()
        if ok: st.success("Calendar probe OK ✅"); st.write(res[:500] + ("..." if len(res)>500 else ""))
        else: st.error(f"Calendar probe failed: {res[:500]}")

    if cD.button("Start Export (async)"):
        try:
            export_id = client.export_start({"params": params})
            st.info(f"Export started: {export_id}")
            st.caption("Use 'Poll Export' to check status.")
        except Exception as ex:
            st.error(f"Export start failed: {ex}")

    if cE.button("Poll Export / Download"):
        try:
            info = client.export_poll_and_maybe_download()
            st.write(info)
        except Exception as ex:
            st.error(f"Export poll/download failed: {ex}")

    st.divider()
    st.markdown("### Troubleshooting Console")
    st.caption("Runs a matrix of requests varying Auth (Header/Query) × Param sending (JSON/Query/Form). Uses pageSize=1, minimal columns, your filterId/project/JQL.")
    if st.button("Run Matrix Test"):
        attempts = client.matrix_test(base_params=params, override_page_size=1)
        df = pd.DataFrame([a.to_row() for a in attempts])
        st.dataframe(df)
        # Save log for download
        raw = [a.to_dict() for a in attempts]
        b = json.dumps(raw, indent=2).encode("utf-8")
        st.download_button("Download Debug Log (JSON)", b, "timepiece_debug_log.json", "application/json")

    st.markdown("### Exact cURL Preview")
    curl = client.curl_preview("/rest/aggregation", params, accept="application/json")
    st.code(curl)
