...
# ===================== TABS =====================
tabs = st.tabs(["Overview", "Cycle Time", "Slowest Items", "Forecasting", "Data", "Sprint Analysis"])
...
if sprint_tab is not None:
    with sprint_tab:
        ...
        # Select a dataframe filtered by Team (as used elsewhere)
        ...
        if df_src is None or df_src.empty or "Sprint" not in df_src.columns or "IssueType" not in df_src.columns:
            ...
            # Normalize Sprint to a list and explode
            ...
            if expanded.empty:
                ...
                # Determine recency (latest End or Resolved per sprint)
                end_col = "End" if "End" in expanded.columns else ("Resolved" if "Resolved" in expanded.columns else None)
                if end_col is not None:
                    expanded["_enddate"] = pd.to_datetime(expanded[end_col], errors="coerce")
                else:
                    expanded["_enddate"] = pd.NaT
                ...
                # Compact summary strings (sorted by % desc)
                compact = (
                    merged.sort_values(["Sprint", "Percent"], ascending=[True, False])
                          .groupby("Sprint")
                          .apply(lambda g: ", ".join([f"{it}: {p:.0f}%" for it, p in zip(g["IssueType"], g["Percent"])]))
                          .rename("Composition")
                          .reindex(sprint_order)
                )

                # Carryover analysis: items that span more than 1 sprint
                if "Key" in expanded.columns:
                    spans = expanded.groupby("Key")["Sprint"].nunique()
                    carryover_keys = set(spans[spans > 1].index.tolist())
                    carry_rows = []
                    for spr in sprint_order:
                        spr_df = expanded[expanded["Sprint"] == spr]
                        total_items = len(spr_df)
                        co_items = spr_df["Key"].isin(carryover_keys).sum() if total_items > 0 else 0
                        co_pct = (co_items / total_items * 100) if total_items > 0 else 0.0
                        carry_rows.append({"Sprint": spr, "Total Items": total_items, "Carryover Items": co_items, "Carryover %": round(co_pct, 1)})
                    carry_df = pd.DataFrame(carry_rows)
                else:
                    carry_df = pd.DataFrame(columns=["Sprint", "Total Items", "Carryover Items", "Carryover %"])

                # --- UI ---
                st.markdown("**Carryover (items spanning >1 sprint)**")
                if not carry_df.empty:
                    carry_df["Sprint"] = pd.Categorical(carry_df["Sprint"], categories=sprint_order, ordered=True)
                    carry_df = carry_df.sort_values("Sprint").reset_index(drop=True)
                    st.dataframe(carry_df, use_container_width=True)
                    # Simple bar chart for carryover % by sprint
                    st.bar_chart(carry_df.set_index("Sprint")["Carryover %"])
                else:
                    st.info("No carryover items detected.")

                st.markdown("**Sprint breakdown — click a sprint to expand**")
                # Drill-down: one expander per sprint row
                for spr in sprint_order:
                    # Build label: Sprint — Compact composition
                    ...
                    with st.expander(label):
                        ...
                        g = g.sort_values("Percent", ascending=False)
                        g["Percent"] = g["Percent"].round(1)
                        st.dataframe(g[["IssueType", "Count", "Percent"]].reset_index(drop=True), use_container_width=True)

                        # Optional: highlight carryover items list (top 20)
                        if "Key" in expanded.columns and len(carryover_keys) > 0:
                            spr_keys = expanded[expanded["Sprint"] == spr]["Key"]
                            co_keys = [k for k in spr_keys.unique() if k in carryover_keys]
                            if co_keys:
                                st.caption(f"Items spanning multiple sprints in {spr} (showing up to 20):")
                                st.write(", ".join(co_keys[:20]))
