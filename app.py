...
team_options = ["All Teams"] + list(team_rules.keys())
selected_team = st.selectbox("Select Team", team_options)

# Apply team filter
if selected_team != "All Teams":
    pattern = team_rules[selected_team]
    filtered_df = df[df['Key'].str.contains(pattern, na=False)]
else:
    filtered_df = df.copy()
...
def parse_team_mapping(df: pd.DataFrame, built_in_team_col: Optional[str], key_col: str, rules_text: str) -> pd.DataFrame:
    out = df.copy()
...
raw = parse_team_mapping(raw, built_in_team_col, key_col, rules_text)
...
# Team selector (sidebar, classic)
with st.sidebar:
    teams = sorted(raw["Team"].dropna().unique().tolist() or ["Unassigned"])
    sel_team = st.selectbox("Team", options=["All Teams"] + teams, index=0)
    st.caption(f"Using **{ct_label}** for all charts.")

view_df = raw if sel_team == "All Teams" else raw[raw["Team"] == sel_team].copy()
...
# ---------- OVERVIEW ----------
with tabs[0]:
    st.subheader("Overview" + ("" if sel_team == "All Teams" else f" — {sel_team}"))
...
# ---------- TEAMS ----------
with tabs[1]:
    st.subheader("Team deep-dive" + ("" if sel_team == "All Teams" else f" — {sel_team}"))
...
with tabs[2]:
    ...
    if sel_team == "All Teams":
        st.info("Select a single team in the sidebar to run a per-team forecast.")
    else:
