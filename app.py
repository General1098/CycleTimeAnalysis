...
if sprint_tab is not None:
    with sprint_tab:
        ...
        # Choose the dataframe currently filtered by Team (view_df if available)
        ...
        if df_src is None or df_src.empty or "Sprint" not in df_src.columns or "IssueType" not in df_src.columns:
            ...
            expanded = expanded.assign(Sprint=expanded["Sprint"].astype(str).str.split("," )).explode("Sprint")
            ...
            if expanded.empty:
                ...
                # Determine recency from End/Resolved
                ...
                # Pivot for a per-sprint view with issue types as columns (%)
                pivot_pct = merged.pivot_table(index="Sprint", columns="IssueType", values="Percent", aggfunc="sum", fill_value=0)
                pivot_pct = pivot_pct.reindex(sprint_order)
                pivot_pct = pivot_pct.round(1)

                # Add total item count per sprint
                totals_idx = totals.set_index("Sprint")["Total"]
                pivot_pct.insert(0, "Total Items", totals_idx.reindex(pivot_pct.index))

                st.markdown("**Percent composition by sprint (%)**")
                st.dataframe(pivot_pct.reset_index(), use_container_width=True)

                # Optional compact summary line per sprint (sorted by % desc)
                comp = (merged.sort_values(["Sprint", "Percent"], ascending=[True, False])
                              .groupby("Sprint")
                ...
                # Detailed long table (for reference)
                merged["Percent"] = merged["Percent"].round(1)
                merged["Sprint"] = pd.Categorical(merged["Sprint"], categories=sprint_order, ordered=True)
                merged = merged.sort_values(["Sprint", "Percent"], ascending=[True, False]).reset_index(drop=True)
                with st.expander("Show detailed breakdown"):
                    st.dataframe(merged[["Sprint", "IssueType", "Count", "Total", "Percent"]], use_container_width=True)
