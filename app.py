...
with tabs[0]:
    ...
    # === Enhanced CT Trends (Monthly vs Rolling, provisional month, P50/Mean toggle, counts) ===
    def _build_ct_series_from_view(df: pd.DataFrame, tz: str = "Europe/London", rolling_days: int = 30, min_n_for_pct: int = 20):
        ...
        d["completed_date"] = d["completed_date"].dt.tz_convert("Europe/London")

        # Monthly
        d["month"] = d["completed_date"].dt.to_period("M").dt.to_timestamp().dt.tz_localize("Europe/London")
        monthly = (
        ...
        now_tz = pd.Timestamp.now(tz="Europe/London")
        ...
        day_index = pd.date_range(start=start_date, end=end_date, freq="D", tz="Europe/London")

        means, medians, p85s, counts = [], [], [], []
        for day in day_index:
            window_start = day - pd.Timedelta(days=30 - 1) if rolling_days is None else day - pd.Timedelta(days=rolling_days - 1)
        ...
        rolling = pd.DataFrame({"date": day_index, "mean_ct": means, "median_ct": medians, "p85_ct": p85s, "count": counts})
