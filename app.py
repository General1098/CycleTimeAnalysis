...
def fib_overlay_chart(dates, values, title="P85 Cycle Time with Fibonacci Bands"):
    ...
    # Shaded zones
    ax.axhspan(fib_vals[2], fib_vals[0], color="#ffebee", alpha=0.7)   # red zone (worst)
    ax.axhspan(fib_vals[3], fib_vals[2], color="#fff8e1", alpha=0.7)   # orange
    ax.axhspan(fib_vals[4], fib_vals[3], color="#f1f8e9", alpha=0.7)   # yellow/green
    ax.axhspan(fib_vals[-1], fib_vals[4], color="#e8f5e9", alpha=0.7)  # green (best)
        ...
        if eview == "Monthly":
            ...
            if monthly.empty:
                ...
                if use_fib_overlay and metric == "P85":
                    ...
                    # Rebuild TRUE monthly aggregation from your existing monthly dataframe
                    df_plot = (
                        monthly
                    ...
                    # Convert months to timestamps Matplotlib can understand
