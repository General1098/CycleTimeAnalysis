...
with tabs[5]:
    ...
    if view_df.empty or "Sprint" not in view_df.columns or "IssueType" not in view_df.columns:
        st.info("No sprint/issue type data available.")
    else:
        # Expand items that belong to multiple sprints
        expanded = view_df.copy()
        expanded["Sprint"] = expanded["Sprint"].fillna("")
        expanded = expanded.assign(Sprint=expanded["Sprint"].str.split(", ")).explode("Sprint")
        expanded = expanded[expanded["Sprint"].str.strip() != ""]

        if expanded.empty:
            st.info("No sprint data after expansion.")
        else:
            # Use End date as proxy for sprint recency
            expanded["_enddate"] = pd.to_datetime(expanded["End"], errors="coerce")
            sprint_order = (
            ...
            merged = pd.merge(grouped, totals, on="Sprint")
            merged["Percent"] = (merged["Count"] / merged["Total"] * 100).round(1)

            # Sort sprint order by recency
            merged["Sprint"] = pd.Categorical(merged["Sprint"], categories=sprint_order, ordered=True)
            merged = merged.sort_values(["Sprint", "IssueType"])
