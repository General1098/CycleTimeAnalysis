...
def build_cycle_time_timeseries(
    ...
    # Build a daily index across the completion span
...
def load_completed_items(team: Optional[str], start: Optional[datetime], end: Optional[datetime]) -> pd.DataFrame:
    ...
    if TIMEPIECE_AVAILABLE:
        ...
        # Fallback: empty DataFrame (app will still render)
        return pd.DataFrame(columns=[\"completed_date\", \"cycle_time_days\", \"team\"]).head(0)
...
def render_cycle_time_trend(df_done: pd.DataFrame):
    ...
    # KPI tiles (last point of rolling-30 preferred)
    try:
    ...
    if view == \"Monthly\":
        ...
        if show_counts:
            # Overlay faint bars and labels for counts
            bars = base.mark_bar(opacity=0.2).encode(
                y=alt.Y(\"count:Q\", title=\"Items (count)\", axis=alt.Axis(titleColor=\"#666\")),
            )
        ...
        # Insight note for small samples
...
def layout_sidebar():
    ...
    # Default date window: last 180 days
    today = datetime.now()
...
def main():
    ...
    # Tabs (Cycle Time Trend tab first; others can be added later)
