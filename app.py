...
with tabs[0]:
    ...
    # === Enhanced CT Trends (Monthly vs Rolling, provisional month, median toggle, counts) ===
    def _build_ct_series_from_view(df: pd.DataFrame, tz: str = "Europe/London", rolling_days: int = 30):
        ...
        rolling = pd.DataFrame({"date": day_index, "mean_ct": means, "median_ct": medians, "count": counts})
        return {"monthly": monthly, "rolling": rolling}

    # Controls for enhanced trends
    ec1, ec2, ec3, ec4 = st.columns([1.3, 1, 1, 1.35])
    eview = ec1.selectbox("Time grouping (enhanced)", ["Monthly", "Rolling (30 days)", "Rolling (14 days)"], index=0, key="ctenh_view")
    euse_median = ec2.checkbox("Show median", value=True, key="ctenh_median")
    ...
    # KPI tiles: last point of rolling-30
    ...
    if eview == "Monthly":
        ...
        if monthly.empty:
            ...
            y_field = "median_ct" if euse_median else "mean_ct"
            y_title = "Median CT (days)" if euse_median else "Average CT (days)"
        ...
        if rolling.empty:
            ...
            y_field = "median_ct" if euse_median else "mean_ct"
            y_title = f"{'Median' if euse_median else 'Average'} CT (days) — trailing {ewindow}d"
    ...
    st.caption("Note: Current-month values can appear artificially low early in the month due to small sample sizes. Use Rolling 30d mid-month for a truer trend.")
