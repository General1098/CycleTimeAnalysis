...
def compute_ct_both(df: pd.DataFrame, started_col: str, finished_col: str) -> pd.DataFrame:
    ...
    # Business days (Mon–Fri), not counting weekends
    ...
    # Calendar days, counting both start & finish days
...
with tabs[0]:
    ...
    team_df = raw[raw["Team"] == sel_team].copy()
...
with tabs[3]:
    ...
    st.dataframe(raw.assign(DateIssue=raw["_date_issue"]).head(200))
